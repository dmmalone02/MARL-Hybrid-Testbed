...
class RandomWalkStep(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('turn_angle', 85.0)   # degrees per left/right turn (default 85)
    ...
    def start_turn_left(self):
        turn_deg = float(self.get_parameter('turn_angle').value)
        self.get_logger().info(f"TURN LEFT  {turn_deg} °")
        self._reset_rotation(math.radians(turn_deg))
        self.state = State.ROTATE

    def start_turn_right(self):
        turn_deg = float(self.get_parameter('turn_angle').value)
        self.get_logger().info(f"TURN RIGHT  {turn_deg} °")
        self._reset_rotation(math.radians(-turn_deg))
        self.state = State.ROTATE
    ...
    def start_move_backward(self):
        ...
        turn_deg = float(self.get_parameter('turn_angle').value)
        flip_deg = 175.0  # backward flip angle
        self.get_logger().info(f"BACKWARD  (flip {flip_deg}° → drive)")
        self._reset_rotation(math.radians(flip_deg))
        self.state = State.ROTATE
