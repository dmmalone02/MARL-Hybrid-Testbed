...
def start_jammer(args):
    if args.jammer_node is None:
        return None

    cmd = [
        "ssh",
        "-n",
        f"ucanlab@10.1.1.{args.jammer_node}",
        "python3",
        "-u",
        "/home/ucanlab/Downloads/RF_Jammer.py",
        "-t",
        str(args.jam_noise_amp),
        "-g",
        str(args.jam_tx_gain),
    ]

    print(f"Starting RF jammer on node {args.jammer_node}")
    print(" ".join(cmd))
...
def stop_jammer(args):
    ...
    print(f"Stopping RF jammer on node {args.jammer_node}")
    subprocess.run(cmd)
...
def main():
    ...
    parser.add_argument("--jammer_node", type=int, default=None)
    parser.add_argument("--jam_start_ep", type=int, default=None)
    parser.add_argument("--jam_stop_ep", type=int, default=None)
    parser.add_argument("--jam_noise_amp", type=float, default=10)
    parser.add_argument("--jam_tx_gain", type=float, default=50)
    ...
    for tx_node, trigger_mode in zip(args.tx_nodes, args.rf_trigger_modes):
        tx_host = f"10.1.1.{tx_node}"
        tx_url = f"http://{tx_host}:8080/"
        print(f"Connecting to TX node {tx_node}: {tx_url}")
        tx = ServerProxy(tx_url)
        tx_sequence.append((tx_node, tx, trigger_mode))
    ...
    try:
        for ep in range(1, args.episodes + 1):
            ...
            if args.jammer_node is not None and args.jam_start_ep is not None:
                if ep >= args.jam_start_ep and not jammer_running:
                    jammer_proc = start_jammer(args)
                    jammer_running = True

                if args.jam_stop_ep is not None and ep > args.jam_stop_ep and jammer_running:
                    stop_jammer(args)
                    jammer_running = False
                    jammer_proc = None
                    time.sleep(2)
        ...
        if jammer_proc is not None:
            try:
                jammer_proc.terminate()
            except Exception:
                pass
